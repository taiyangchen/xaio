 ...
 */
public class RpcTest extends LoggerSupport {

  private DefaultAsyncServer server;

  private DefaultAsyncClient client;

  private Helloworld helloworldApi;
...
//    client.setReceiver(receiver);
  ...
  private void initApi(DefaultAsyncClient client) throws Exception {
    RpcProxyCreator rpc = new RpcProxyCreator();
    rpc.setPeer(client);
    rpc.setServiceInterfaces(new Class<?>[] {Helloworld.class});
  ...
  private void doTest() {
    try {
      String name = helloworldApi.getName("test");
  ...
  public void testUserInfo() {
    for(int i=0;i<100;i++) {
      ...
      UserInfo result = helloworldApi.echo(user);
...
//      }
      for (int i = 0; i < 50000; i++) {
        String value = helloworldApi.getName("test : " + i);
